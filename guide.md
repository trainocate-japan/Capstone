...
## 演習概要
- 要件定義で提示された要件をもとに設計を行い、クラウド上にアプリ開発チームが開発したWebアプリケーションを展開する演習です
...
- その他の役割には、構築担当、ドキュメント担当などがあります。メンバーの手空きの時間がでないよう、役割を決めましょう
...
- テストができる段階になったら、作成済みのテスト項目に沿ってテストを行い、品質を満たしているかを確認してください。テストの開始は全ての構築が完了するまで待つ必要がありませんが、手戻りが発生しないように効率よく進めましょう
...
- AWSを使用すること
- アプリケーションコードは、GitLab でソース管理すること
- GitLab から CI/CD パイプラインを用いて自動テスト、自動デプロイを行うこと
- （オプション）Docker コンテナとしてアプリケーションをサーバーにデプロイする
...
- アプリケーションサーバとデータベースサーバへのインターネットからの直接アクセスは厳禁とする
	- ただしパッチ適用などのため、インターネットへの疎通は確保すること
- インターネットから全サーバへの直接のSSHは厳禁とする
- SSH接続のために踏み台となるサーバを用意すること。踏み台サーバへは、チームメンバーのアドレスからのみログインできるようにファイアウォールを構成すること
- アプリケーション・サーバーは、GitLab Runner からの SSH 接続を受付可能であること
...
- アプリケーションの動作するサーバはCPU使用率が50%を超えないように自動スケールすること
- 自動スケールのテスト結果を報告書にまとめ提示すること

#### 運用保守性
- リソース監視・ログ監視にはZabbixを用いること
- インフラに発生する各種障害はZabbixにより検知し、必要に応じてアラートが発報されること
- CloudWatch で代替することも可能とする
- インフラへのアプリケーションのデプロイは、CI/CD パイプラインを実行すること
...
### スケジュール例（インフラ）
1日目：チームビルディング、要件整理、アーキテクチャ設計
2日目：アーキテクチャ設計、アーキテクチャレビュー（講師）、テスト仕様書作成
3日目：テスト仕様書作成、テスト仕様書レビュー、構築
4日目：構築
5日目：構築、テスト（CD 経由でアプリも入ってくる頃合い）
6日目：テスト、ドキュメント作成
7日目：報告会資料作成、報告会
